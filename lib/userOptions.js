...
// async function to handle viewing employees by department
const viewByDepartment = async (initialQuestion) => {
  return new Promise((resolve, reject) => {
    // because the employee table is self-referencing use two versions of the table, e and m, to handle the employee names and the manager names and then left join
    const sql = `SELECT employee.id AS id, CONCAT(employee.first_name, " ", employee.last_name) AS  employee, department.name AS department
    FROM employee
    JOIN role
...
// async function to handle viewing the total utilized budget by department
const viewTotalUtilizedBudget = async (initialQuestion) => {
  //create an array of the departments for the user to choose from
  let departmentList = await getDepartments();
  ...
  return new Promise((resolve, reject) => {
    inquirer
      ...
      .then((response) => {
        let salaries = [];
        const sql = `SELECT employee.id AS id, employee.last_name AS last_name, role.title AS title, department.name AS department, role.salary AS salary
        FROM employee
        JOIN role
        ON employee.role_id = role.id
        JOIN department
        ...
        db.query(sql, params, (err, result) => {
          ...
          // create an array of the salaries as integer values
          for (var i = 0; i < result.length; i++) {
            salaries.push(parseInt(result[i].salary));
          }
          //create a sum variable and add them together to create the total budget
          let sum = 0;
          for (let i = 0; i < salaries.length; i++) {
            sum += salaries[i];
          }

          //console the sum as the budget for the given department
          console.log("----------------------------------");
          resolve(
            console.log(
